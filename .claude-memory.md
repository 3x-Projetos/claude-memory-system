...
### Quick Win Framework
**Uso**: Priorizar tarefas baseado em benefício vs dificuldade

**Critérios**:
- **Benefício**: Impacto/ganho da tarefa (Alto/Médio/Baixo)
- **Dificuldade**: Tempo e complexidade de implementação (Alta/Média/Baixa)

**Matriz de Priorização**:
1. 🎯 **Quick Wins** (Alto benefício + Baixa dificuldade) → PRIORIDADE MÁXIMA
2. 📈 **Major Projects** (Alto benefício + Alta dificuldade) → Planejar bem
3. 🔧 **Fill Ins** (Baixo benefício + Baixa dificuldade) → Quando sobrar tempo
4. ❌ **Time Wasters** (Baixo benefício + Alta dificuldade) → Evitar

**Exemplo aplicado**:
- SessionEnd hook: Alto benefício (nunca perde sessão) + Baixa dificuldade (5-10 min) = 🎯 Quick Win

**Quando usar**: Ao planejar implementações ou decidir próximos passos
...
### `/continue`
**Uso recomendado para retomar trabalho**. Carrega ferramentas e contexto da última sessão, assume que vai continuar de onde parou. Apresenta pendências e continua direto.

### `/new`
**Uso recomendado para nova atividade**. Carrega ferramentas e awareness da última sessão, mas foca em começar algo novo. Não lista pendências.
...
### `/auto-approve`
**Toggle em tempo real de auto-aprovação com múltiplos níveis.**
...
Controla quais operações requerem aprovação manual. Mudanças aplicam instantaneamente.

**Níveis disponíveis**:
- `off` - Nada aprovado automaticamente
- `edits` - Apenas Edit e Write
- `bash` - Edit, Write e Bash (padrão recomendado para fluxo fluido)
- `all` - Todos os tools (máxima automação)

---

## Auto-Aprovação com Múltiplos Níveis

**Status**: ✅ Configurado com toggle em tempo real e múltiplos níveis

**Arquivos**:
- `.claude/settings.json` - Configuração local do hook
- `.claude/auto-approve-state` - Arquivo de estado (off/edits/bash/all)
- `.claude/auto-approve-edits.py` - Script PreToolUse com lógica de níveis
- `.claude/AUTO-APPROVE-README.md` - Documentação completa

**Propósito**: Elimina prompts de aprovação manual para operações selecionadas, com controle granular.
...
2. **Padrão recomendado**: `bash` (já configurado)
   - Aprova: Edit, Write, Bash
   - Fluxo de trabalho fluido sem interrupções
3. **Outros níveis disponíveis**:
   - `/auto-approve edits` → apenas Edit/Write
   - `/auto-approve all` → todos os tools (máxima automação)
   - `/auto-approve off` → desativa tudo
4. **Verificar**: Use `/auto-approve status` para ver nível atual

**Toggle em tempo real**: Hook lê arquivo de estado a cada operação, permitindo mudar de nível sem reiniciar CLI.

**Escopo**: Funciona APENAS neste diretório (não afeta outros projetos).

**Importante**: Configuração LOCAL (`.claude/settings.json`), não global. Portável e isolada por projeto.

---

## Graceful Shutdown (SessionStart/SessionEnd Hooks)

**Status**: ✅ Implementado com rastreamento por sessão

**Propósito**: Garantir que **nenhuma sessão seja perdida**, mesmo se você esquecer de executar `/end` antes de encerrar o CLI (Ctrl+C).

**Como funciona**:
1. **SessionStart hook** (`.claude/session-start.py`):
   - Executa automaticamente ao iniciar CLI
   - Cria arquivo `.claude/.current-session-id` com timestamp único
   - Marca início desta sessão específica

2. **Durante a sessão**:
   - Se você usa `/end` → comando deleta `.current-session-id` (marca como logged)
   - Se você não usa `/end` → arquivo permanece
...
3. **SessionEnd hook** (`.claude/session-auto-end.py`):
   ...
   - Verifica se `.current-session-id` existe:
     - **NÃO existe** → `/end` foi executado → não faz nada ✅
     - **EXISTE** → sessão não foi logged → cria registro automático ✅

**Múltiplas sessões no mesmo dia**:
```
Manhã:  Sessão 1 → /end → log completo ✅
        SessionEnd → detecta logged → não faz nada

Tarde:  Sessão 2 → esquece /end
        SessionEnd → detecta não-logged → append ao log do dia ✅
```

**Arquivos**:
- `.claude/session-start.py` - Marca início da sessão
- `.claude/session-auto-end.py` - Backup automático ao encerrar
- `.claude/.current-session-id` - Estado da sessão atual (criado/deletado automaticamente)
- `.claude/settings.json` - Hooks SessionStart e SessionEnd registrados

**Benefícios**:
- ✅ Zero fricção (funciona silenciosamente)
- ✅ Nunca perde sessão (sempre haverá registro)
- ✅ Suporta múltiplas sessões por dia
- ✅ Não interfere com `/end` manual
- ✅ Continuidade garantida via `/continue`

**Ativação**: Requer reiniciar Claude CLI uma vez. Após isso, funciona automaticamente em todas as sessões.
...
**Como usar**:
```bash
# Opção 1: Executar direto deste diretório
bash .claude/setup-claude-memory.sh /caminho/do/novo/projeto

# Opção 2: Copiar script e executar lá
...
**O que faz**:
- Cria toda estrutura (memória, workflows, comandos)
- Instala documentação completa
- Sistema fica idêntico ao deste diretório
- Pronto para usar após reiniciar Claude CLI

**Após instalação**:
1. Reiniciar Claude CLI
2. Usar `/start` para começar
...
**Status**: ✅ Implementado (FASE 0-6 completas)
**Versão**: 2.0

### Arquitetura

Sistema completo de memória multi-camadas com agregação temporal para economia de tokens e personalização contínua.

**Memória Global** (multi-projeto):
- Localização: `~/.claude-memory/`
- Perfil do usuário versionado
- Compartilhado entre todos os projetos Claude
- PII protegido via redação automática

**Memória Local** (por projeto):
- Working memory: `.session-state.md` (~50 linhas)
- Logs diários: `logs/daily/YYYY.MM.DD.md` (detalhado)
...
├── weekly/                    # Agregações semanais
└── monthly/                   # Agregações mensais
...
### Workflows de Agregação

**4 Workflows documentados**:
1. `.workflow-weekly-aggregation.md` - Dia → Semana (~85% economia)
2. `.workflow-monthly-aggregation.md` - Semana → Mês (~93% economia vs diários)
3. `.workflow-profile-update.md` - Atualização de perfil (mensal ou trigger-based)
4. `.workflow-metrics-collection.md` - Métricas holísticas (7 dimensões)

### Comandos Slash Atualizados

**9 Comandos disponíveis**:
- `/start` - Pergunta se continua ou inicia novo
- `/continue` - Retoma de onde parou (carrega hierarquia + pendências)
- `/new` - Nova atividade (carrega hierarquia, sem pendências)
- `/memory` - Consulta ferramentas disponíveis
- `/organize` - Organiza notas
- `/end` - Finaliza sessão (logs + métricas + session-state)
- `/update-profile` - Atualiza perfil global (manual)
- `/reflect` - Coleta métricas de well-being
- `/aggregate` - Força agregação semanal/mensal
...
- Dados redacted em transmissão (`.safe.md`)

### Framework de Métricas Holísticas

**Arquivo**: `.claude/METRICS-FRAMEWORK.md`

**7 Dimensões de Impacto Humano**:
...
**Coleta**:
- **Inferidos**: Análise automática de logs (complexity, velocity, tech exposure)
- **Calculados**: Derivados (time saved, autonomy growth)
- **Relatados**: Via `/reflect` (satisfaction, energy, frustration)

**Agregação**: Métricas aparecem em logs diários, resumos semanais e mensais
...
**Versionamento**:
- Semântico: Major.Minor (ex: 2.1)
- Snapshots em `profile-history/`
- Changelog completo em `profile-changelog.md`

**Atualização**:
- Mensal (automático via `/continue`)
- Threshold (3+ menções/semana de mudança)
...
**Início de sessão**:
```
/continue  → Retoma trabalho (pendências listadas)
/new       → Nova atividade (sem pressão de pendências)
```

**Durante sessão**:
```
/reflect   → Registra well-being (opcional)
/organize  → Processa notas
```

**Fim de sessão**:
```
/end       → Cria log + métricas + atualiza session-state
```

**Manutenção**:
```
/aggregate week      → Toda segunda-feira (recomendado)
/aggregate month     → Todo dia 1 do mês (recomendado)
/update-profile      → Quando detectar mudanças significativas
```

### Benefícios

**Economia de Tokens**:
- Carregamento típico: ~330 linhas (hierarquia)
- Vs logs brutos: ~5.000+ linhas
- **Economia: ~93%**

**Personalização Contínua**:
- Claude aprende suas preferências ao longo do tempo
- Adapta colaboração ao seu working style
- Sugere melhorias baseadas em padrões

**Insights Holísticos**:
- Tracking de impacto humano (além de produtividade)
- Tendências de well-being, aprendizado, autonomia
...
**Tipo**: Obsidian Vault
**Uso**: Notas pessoais, organização de ideias, workflows de criação
**Tópicos principais**: Creative workflow, image generation, LLM infrastructure, scene development

...
2. **Escolher nível**:
   - `/auto-approve on` → modo edits (recomendado para trabalho normal)
   - `/auto-approve bash` → modo bash (quando precisa rodar comandos)
   - `/auto-approve all` → modo all (máxima automação, use com cautela)
   - `/auto-approve off` → desativa tudo
3. **Verificar**: Use `/auto-approve status` para ver nível atual
